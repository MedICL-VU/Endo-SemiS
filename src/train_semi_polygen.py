...
def train(args, train_loader_labeled, train_loader_unlabeled, model1, model2, model3, criterion,
    ...
    for batch_labeled, batch_unlabeled in loader:
        ...
        # input_labeled_patch, target_labeled_patch = split_tensor_into_patches(input_labeled), split_tensor_into_patches(target_labeled)
        # classification_labels_patch = (target_labeled_patch.view(target_labeled_patch.size()[0], -1).sum(dim=1) > 0).float().unsqueeze(1)
        # output_patch2 = model2(input_labeled_patch)
        # output_patch2_c = reconstruct_from_patches(output_patch2, batch_size=args.labeled_batch_size)
...
if __name__ == '__main__':
    ...
    dataset_labeled = get_dataset(args, data='polygen', mode='train', labeled=True, sup=False)
    dataset_unlabeled = get_dataset(args, data='polygen', mode='train', labeled=False, sup=False, w_s=False, unimatch=True)
    valset = get_dataset(args, data='polygen', mode='val')

    # net1 = get_net(args, net=args.net1)
    # net2 = get_net(args, net=args.net2)
