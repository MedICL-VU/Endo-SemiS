...
def validate(net1, net2, loader, device, activation=nn.Sigmoid(), save=False):
    ...
    with torch.no_grad():
        with tqdm(total=len(loader), desc='Validation round', unit='batch', leave=False) as pbar:
            for i, (batch) in enumerate(loader):
                ...
                if save:
                    ...
                    save_dir_image = os.path.join(save_base_dir, args.name, 'image', save_dataset)
                    ...
                    image_save = input.squeeze(0).permute(1, 2, 0).detach().cpu().numpy()
                    ...
                    np.save(os.path.join(save_dir_image, save_name), image_save)
        ...
        logging.info(
            'Model 1+2, batch-wise validation accuracy: {}, precision: {}, recall: {}, f1: {}'.format(accuracy_3,
                                                                                                      precision_3,
                                                                                                      recall_3,
                                                                                                      f3))
        ...
        logging.info('Model 1, batch-wise validation sensitivity coeff: {}, std: {}, median: {}'.format(np.mean(sen1),
                                                                                                        ...
                                                                                                        np.median(
                                                                                                            sen1)))
        logging.info('Model 2, batch-wise validation sensitivity coeff: {}, std: {}, median: {}'.format(np.mean(sen2),
                                                                                                        np.std(sen2),
                                                                                                        np.median(
                                                                                                            sen2)))
        logging.info('Model 1+2, batch-wise validation sensitivity coeff: {}, std: {}, median: {}'.format(np.mean(sen3),
                                                                                                        np.std(sen3),
                                                                                                        np.median(
                                                                                                            sen3)))
        ...
        logging.info('Model 1, batch-wise validation specificity coeff: {}, std: {}, median: {}'.format(np.mean(spe1),
                                                                                                        ...
                                                                                                        np.median(
                                                                                                            spe1)))
        logging.info('Model 2, batch-wise validation specificity coeff: {}, std: {}, median: {}'.format(np.mean(spe2),
                                                                                                        np.std(spe2),
                                                                                                        np.median(
                                                                                                            spe2)))
        logging.info('Model 1+2, batch-wise validation specificity coeff: {}, std: {}, median: {}'.format(np.mean(spe3),
                                                                                                          np.std(spe3),
                                                                                                          np.median(
                                                                                                              spe3)))
...
if __name__ == '__main__':
    ...
    dataset = PolyGenData_semi(args, mode='test', transform=Compose([Resize(args.height, args.width),
